...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (isActive && timer > 0) {
      ...
      triggerModal("WAKTU HABIS!", "Sesi fokus kamu berakhir.", true);
    }
  ...
  async function addTask(e: React.FormEvent) {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return toast.error("Sesi habis, login ulang Fa!");
  ...
  async function handleShare() {
    if (!shareEmail.includes('@')) return toast.error("Email gak valid!");
          ...
          {showShareModal && (
            <div className="relative bg-slate-900 border border-white/10 w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl text-white">
              ...
              <div className="space-y-6">
                <input type="email" placeholder="Email teman..." value={shareEmail} onChange={(e) => setShareEmail(e.target.value)} className="w-full bg-slate-950/50 border border-white/5 px-6 py-4 rounded-2xl outline-none text-white"/>
          ...
          {isEditingProfile ? (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={updateProfile} className="flex-1 bg-indigo-600 p-2 rounded-xl text-[10px] font-bold">Save</button>
            ...
            <div className="flex items-center justify-between group">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center font-black text-white shrink-0">{profileName[0]?.toUpperCase()}</div>
              ...
              <button onClick={() => setIsEditingProfile(true)} className="p-2 hover:bg-white/10 rounded-lg text-slate-500"><Edit3 size={16}/></button>
            </div>
          )}
        </div>

        <nav className="flex-1 space-y-2 overflow-y-auto">
          <p className="text-[10px] font-black opacity-20 uppercase tracking-[0.3em] px-4 mb-4 text-left font-sans">Workspace</p>
          <button onClick={() => {setFilterCat('All'); setIsSidebarOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${filterCat === 'All' ? 'bg-indigo-600 shadow-lg text-white' : 'hover:bg-white/5 text-slate-400'}`}><LayoutDashboard size={18}/> <span>Dashboard</span></button>
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto font-sans">
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`p-8 rounded-[2.5rem] border transition-all ${isDarkMode ? 'bg-slate-900 border-white/5 shadow-2xl' : 'bg-slate-50 border-slate-200 shadow-sm'}`}>
              <h3 className="text-[10px] font-black opacity-30 mb-2 uppercase text-left tracking-[0.2em] font-sans">Overall Progress</h3>
              <span className="text-5xl font-black text-indigo-500 flex tracking-tighter tabular-nums font-sans">{(tasks.length > 0 ? Math.round((tasks.filter(t => t.status === 'completed').length / tasks.length) * 100) : 0)}%</span>
            </div>
            <div className="p-8 rounded-[2.5rem] text-center flex flex-col items-center justify-center bg-indigo-900/20 border border-indigo-500/20 text-white shadow-xl">
              <span className="text-4xl font-black mb-1 tabular-nums tracking-tighter font-sans">{formatTime(timer)}</span>
              <button onClick={() => setIsActive(!isActive)} className="text-[10px] uppercase font-black tracking-widest text-indigo-400 hover:text-indigo-300 transition-all font-sans">{isActive ? 'Pause Focusing' : 'Start Focus'}</button>
            </div>
          </div>
          ...
          <form onSubmit={addTask} className="bg-slate-900/60 p-8 rounded-[3rem] border border-white/5 shadow-2xl">
            ...
            <div className="flex flex-wrap gap-2 items-center pt-6 border-t border-white/5 text-white">
              <select value={category} onChange={e => setCategory(e.target.value)} className="bg-slate-800 text-white text-[10px] p-3 rounded-xl font-black uppercase tracking-widest outline-none font-sans"><option value="General">General</option><option value="Kerja">Kerja</option><option value="Kuliah">Kuliah</option></select>
              <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="bg-slate-800 text-white text-[10px] p-3 rounded-xl font-black outline-none font-sans" />
              <button type="submit" className="ml-auto bg-indigo-600 text-white px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/20 transition-all font-sans">+ Tambah</button>
            </div>
          </form>
                ...
                <div className="flex items-center gap-5 flex-1 overflow-hidden">
                  ...
                  <div className="flex flex-col text-left overflow-hidden">
                    <div className="flex items-center gap-2">
                      ...
                      {task.description && (
                        <div className="group/info relative flex items-center">
                          ...
                          <div className="absolute bottom-full left-0 mb-3 w-56 p-4 bg-slate-800 text-[10px] text-white rounded-2xl opacity-0 group-hover/info:opacity-100 transition-all z-50 border border-white/10 font-bold shadow-2xl leading-relaxed">
                            {task.description.split(/(https?:\/\/[^\s]+)/g).map((part: string, i: number) => part.match(/(https?:\/\/[^\s]+)/g) ? <a key={i} href={part} target="_blank" rel="noopener noreferrer" className="text-indigo-400 font-bold underline font-sans">Link <ExternalLink size={10}/></a> : part)}
                          </div>
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-3 mt-1.5 overflow-hidden">
                      ...
                      {task.due_date && <span className="text-[9px] text-slate-500 font-bold flex items-center gap-1 uppercase font-sans"><Calendar size={10}/> {task.due_date}</span>}
                    </div>
                  </div>
                </div>
