...
export default function Dashboard() {
  ...
  async function addTask(e: React.FormEvent) {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return toast.error("Login ulang dulu Fa!");
  ...
  return (
    <div className={`min-h-screen flex flex-col lg:flex-row ${isDarkMode ? 'bg-[#0f172a] text-white' : 'bg-white text-slate-900'}`}>
      <Toaster />
      ...
      <aside className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:static inset-0 z-50 w-full lg:w-72 bg-slate-900 text-white p-6 transition-transform duration-300 flex flex-col border-r border-white/5`}>
        ...
        <button onClick={handleLogout} className="flex items-center gap-3 text-[10px] text-red-400 font-black mb-8 hover:bg-red-400/10 p-4 rounded-2xl border border-red-400/20 transition-all uppercase tracking-widest"><LogOut size={16}/> <span>Keluar Aplikasi</span></button>
            ...
            <div className="flex items-center justify-between group">
              <div className="flex items-center gap-3">
                ...
                <div className="overflow-hidden text-left">
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Profile</p>
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto">
          ...
          <button onClick={() => {setFilterCat('All'); setIsSidebarOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${filterCat === 'All' ? 'bg-indigo-600' : 'hover:bg-white/5 text-slate-400'}`}><LayoutDashboard size={18}/> <span>Dashboard</span></button>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`p-8 rounded-[2.5rem] border ${isDarkMode ? 'bg-slate-900 border-white/5 shadow-2xl' : 'bg-slate-50 border-slate-200'}`}>
              ...
              <span className="text-5xl font-black text-indigo-500 flex tracking-tighter">{(tasks.length > 0 ? Math.round((tasks.filter(t => t.status === 'completed').length / tasks.length) * 100) : 0)}%</span>
            </div>
          ...
          <form onSubmit={addTask} className="bg-slate-900/60 p-8 rounded-[3rem] border border-white/5 shadow-2xl">
            ...
            <div className="flex flex-wrap gap-2 items-center pt-6 border-t border-white/5">
              <select value={category} onChange={e => setCategory(e.target.value)} className="bg-slate-800 text-white text-[10px] p-3 rounded-xl font-black uppercase tracking-widest"><option value="General">General</option><option value="Kerja">Kerja</option><option value="Kuliah">Kuliah</option></select>
              <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="bg-slate-800 text-[10px] text-white p-3 rounded-xl font-black" />
              <button type="submit" className="ml-auto bg-indigo-600 text-white px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest">+ Tambah</button>
            </div>
          </form>
          ...
          <div className="space-y-4 pb-20">
            {tasks.filter(t => filterCat === 'All' || t.category === filterCat).map(task => (
              <div key={task.id} className="group p-6 rounded-[2.5rem] bg-slate-900/40 border border-white/5 flex items-center justify-between hover:border-indigo-500/30 transition-all">
                ...
                <div className="flex items-center gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100">
