...
export default function Dashboard() {
  ...
  async function fetchTasks() {
    ...
    if (data) {
      setTasks(data);
      // Hitung streak permanen berdasarkan tugas selesai HARI INI
  ...
  async function toggleStatus(id: string, currentStatus: string) {
    ...
    if (!error) {
      // Refresh streak dari DB agar permanen
      fetchTasks();
    }
  }
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          {showShareModal && (
            <div className="relative bg-slate-900 border border-white/10 w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl text-white">
              ...
              <input type="email" placeholder="Email teman..." value={shareEmail} onChange={(e) => setShareEmail(e.target.value)} className="w-full bg-slate-950/50 border border-white/5 px-6 py-4 rounded-2xl outline-none text-white mb-6"/>
      ...
      <aside className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:static inset-0 z-50 w-full lg:w-72 bg-slate-900 text-white p-6 transition-transform duration-300 flex flex-col border-r border-white/5`}>
          ...
          {isEditingProfile ? (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setIsEditingProfile(false)} className="flex-1 bg-slate-700 p-2 rounded-xl text-[10px] font-bold text-white text-center">No</button>
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-between group">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="overflow-hidden text-left">
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest text-left">Master</p>
        ...
        <div className="mb-8 flex items-center gap-4 bg-orange-500/10 border border-orange-500/20 p-4 rounded-2xl">
           ...
           <div className="text-left">
              ...
              <p className="text-lg font-black text-white italic">{streak} Missions</p>
           </div>
        </div>
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto">
          ...
          {activeCategories.map((cat: any) => (
            <button key={cat} onClick={() => {setFilterCat(cat); setIsSidebarOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${filterCat === cat ? 'bg-indigo-600 text-white' : 'hover:bg-white/5 text-slate-400'}`}><CheckSquare size={18}/> <span>{cat}</span></button>
          ))}
        </nav>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <form onSubmit={addTask} className="bg-slate-900/60 p-8 rounded-[3rem] border border-white/5 shadow-2xl">
            ...
            <div className="flex flex-wrap gap-2 items-center pt-6 border-t border-white/5 text-white">
              <select value={category} onChange={e => setCategory(e.target.value)} className="bg-slate-800 text-white text-[10px] p-3 rounded-xl font-black uppercase tracking-widest outline-none font-bold"><option value="General">General</option><option value="Kerja">Kerja</option><option value="Kuliah">Kuliah</option></select>
              <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="bg-slate-800 text-white text-[10px] p-3 rounded-xl font-black outline-none font-bold" />
              ...
              <div key={task.id} className="group p-6 rounded-[2.5rem] bg-slate-900/40 border border-white/5 flex items-center justify-between hover:border-indigo-500/30 transition-all">
                <div className="flex items-center gap-5 flex-1 overflow-hidden">
                  ...
                  <div className="flex flex-col text-left overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mt-1.5 overflow-hidden text-left">
